...
def update_emo(id):
    global received_messages,total_emotion
    a = Mysqlop()
    for i in range(id,a.get_row_count() + 1):
        received_messages = []
        data_str = a.getData(i-1, 1, "summary")
        main(
            ...
            query=data_str + "对于中国来说，这段文字是积极的、消极的、还是中立的，只给出三者之一,只给出三个字的情感，不用说别的"
        )
    ...
    for index, item in enumerate(total_emotion):
        ...
        print(item)
        print(type(item))
        a.updateSummary('emotion', item, index + id)
